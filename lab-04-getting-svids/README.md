...
[lab-04-getting-svids](../lab-04-getting-svids) as an environment variable:
...
To set sail, spin up your demo Kubernetes cluster using [`Kind`](https://kind.sigs.k8s.io/) by issuing the following `make` command while in
the root [lab directory](/):
...
This command will deploy SPIRE based on the three YAML manifests for the [spire-server](./config/spire-server.yaml),
[spire-agent](./config/spire-agent.yaml), and [spiffe-csi-driver](./config/spiffe-csi-driver.yaml) located within the
[config](./config) directory. We've already covered what the SPIRE Server and Agent do within the previous lab
exercises, but we haven't yet discussed what the `spiffe-csi-driver` is.

The [SPIFFE CSI Driver](https://github.com/spiffe/spiffe-csi), is a [Container Storage Interface (CSI)](https://kubernetes-csi.github.io/docs/) driver
for Kubernetes that facilitates injection of the
[SPIFFE Workload API](https://github.com/spiffe/spiffe/blob/main/standards/SPIFFE_Workload_API.md). This is done by
mounting a directory containing the Workload API Unix domain socket as an
...
container at the requested target path.

This begs the question of what a
...
Now you are ready to write `helper.conf` files for your SPIRE Server and Agent within the [client](./workload/client)
and [server](./workload/server) app directories. To do so, first navigate to the [server](./workload/server) app
directory:
...
Once here, investigate the [Dockerfile](./workload/server/Dockerfile) and [main.py](./workload/server/main.py) to better
understand how we've set up the manifest server using some simple Python code and a docker image. Within the same
directory, create the Server `helper.conf` file by running:

```shell
vi helper.conf
```

Feel free to use an alternative method of file creation if you don't have vi installed, are running on a different
operating system, or prefer another method (e.g. using an IDE). Once the file is created, reference the below example
of a server `helper.conf` for Coastal Containers Ltd to guide your configuration:
...
After you've added the appropriate configuration, and the server `helper.conf` is in place, navigate to the
[client](./workload/client) app directory by running:

```shell
cd $LAB_DIR/workload/client
```

Once here, investigate the [Dockerfile](./workload/client/Dockerfile) and [main.py](./workload/client/main.py) to better
understand how we've set up the manifest client using some simple Python code and a docker image. Within the same
directory, create the client `helper.conf` file by running:
...
Feel free to use an alternative method of file creation if you don't have vi installed, are running on a different
operating system, or prefer another method (e.g. using an IDE). Once the file is created, reference the below example of
a client `helper.conf` for Coastal Containers Ltd to guide your configuration:
...
Alternatively, another sample `helper.conf` file can be found
[here](https://github.com/spiffe/spiffe-helper/blob/main/helper.conf) within the
[SPIFFE Helper](https://github.com/spiffe/spiffe-helper/) GitHub repository. With the configuration files in place,
you can now move on to build and load two workload images using `spiffe-helper` to manage the application's SVIDs.
...
Build the server workload image and load the image into the kind cluster:
...
Build the client workload image and load the image into the kind cluster:
...
To enable the workload pods to consume the crafted server and client `helper.conf` file, you will need to create two
Kubernetes ConfigMaps to hold the configuration for `spiffe-helper`.
...
Optionally, to ensure the `spiffe-helper` config was properly set, inspect the yaml of the newly created config maps by
running:
